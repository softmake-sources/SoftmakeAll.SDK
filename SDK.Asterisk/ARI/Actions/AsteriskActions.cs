...
namespace SoftmakeAll.SDK.Asterisk.ARI.Actions
{
	...
	public class AsteriskActions : ARIBaseAction, IAsteriskActions
	{
		...
		/// <summary>
		/// Gets Asterisk log SoftmakeAll.SDK.Asterisk.ARI.Models.Channel information.. 
		/// </summary>
		...
		/// Adds a log channel.. 
		/// </summary>
		/// <param name="logChannelName">The log SoftmakeAll.SDK.Asterisk.ARI.Models.Channel to add</param>
		...
		public void AddLog(string logChannelName, string configuration)
		{
			...
			switch((int)response.StatusCode)
            {
				case 400:
					throw new AriException("Bad request body", (int)response.StatusCode);
				case 409:
					throw new AriException("Log SoftmakeAll.SDK.Asterisk.ARI.Models.Channel could not be created.", (int)response.StatusCode);
				default:
					// Unknown server response
					throw new AriException(string.Format("Unknown response code {0} from ARI.", response.StatusCode), (int)response.StatusCode);
            }
		...
		public void DeleteLog(string logChannelName)
		{
            ...
            {
				case 404:
					throw new AriException("Log SoftmakeAll.SDK.Asterisk.ARI.Models.Channel does not exist.", (int)response.StatusCode);
				default:
					// Unknown server response
					throw new AriException(string.Format("Unknown response code {0} from ARI.", response.StatusCode), (int)response.StatusCode);
            }
